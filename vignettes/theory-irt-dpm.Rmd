...
> `models-and-workflow` and `posterior-summaries`.
...
$r_p = \sum_{i=1}^{I} y_{ip}$ is a **sufficient statistic** for $\theta_p$.
...
of the parameters leave the likelihood invariant. Without constraints, the
posterior is improper along these directions.
...
`identification` argument to `dpmirt_spec()`:
...
For 2PL/3PL, both $\beta$ (or $\gamma$) and $\log(\lambda)$ are centered:
$$
...
the ability distribution, making it compatible with both Normal and DPM priors.
...
for finite mean ($\nu_1 > 2$) with $\mathbb{E}[\tilde{\sigma}^2] \approx 1$.
...
$$
where $\{\theta^*_h\} \stackrel{iid}{\sim} G_0$. DPMirt uses the
...
> The CB estimator corrects the underdispersion (Goal 3), and the GR
...
simultaneously addresses all three goals: individual estimation, ranking,
and distribution recovery. It is the most computationally intensive of
the three methods but provides the most balanced trade-off.
...
approach: extract DP posterior samples, call NIMBLE's
`getSamplesDPmeasure()` to obtain stick-breaking weights and atoms,
evaluate $f_s(x)$ on a grid, and apply rescaling.
