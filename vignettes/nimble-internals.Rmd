...
comp_table <- data.frame(
  ...
  When_Used = c(
    "constrained_item with 2PL/3PL only",
    "Always (all models, all priors)",
    "2PL/3PL with SI parameterization only"
  ),
...
- WAIC computation (when NIMBLE's built-in WAIC is not available).
...
scale.
To evaluate it on the rescaled theta scale, the grid is shifted by
the iteration-specific location shift:
...
> and recompiles a NIMBLE model to call `getSamplesDPmeasure()`.
> This adds 30--60 seconds.  The `dpmirt()` function runs this
> automatically when `compute_dp_density = TRUE` (the default for
> DPM models).  If you want to skip it, set `compute_dp_density = FALSE`
> and call `dpmirt_dp_density(fit)` later.
...
| WAIC computation failed | NIMBLE version mismatch | Update nimble; use compute\_waic=FALSE |
