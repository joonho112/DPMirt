---
title: "Under the Hood: NIMBLE Backend and Advanced Usage"
author: "JoonHo Lee"
date: "`r Sys.Date()`"
description: >
...
vignette: >
  %\VignetteIndexEntry{Under the Hood: NIMBLE Backend and Advanced Usage}
...
# Note: This vignette uses live calls to dpmirt_simulate() and dpmirt_spec()
# (which are fast), and eval=FALSE for all MCMC-related chunks.
```
...
This vignette is for users who want to:
...
Most users will never need this level of detail --- the `dpmirt()` function
handles everything automatically.
But if you want to write custom samplers, hook into the compiled MCMC,
or adapt DPMirt for a new model, this is where to start.
...
for each model--prior--identification combination.
DPMirt instead generates NIMBLE code programmatically via
`dpmirt_spec()`, dispatching on three configuration axes:

| Axis | Options |
|:-----|:--------|
| Model | rasch, 2pl, 3pl |
| Prior | normal, dpm |
| Identification | constrained\_item, constrained\_ability, unconstrained |

This yields $3 \times 2 \times 3 = 18$ potential combinations (not all
valid), all generated from a single specification call. Current validation
rejects `constrained_ability` with DPM priors and rejects
`constrained_item` for 3PL models.

### Inspecting a Specification

The `dpmirt_spec()` function is lightweight --- it builds the NIMBLE code,
constants, data, initial values, and monitor configuration without
compiling anything.
...
identification has this structure (shown here as pseudocode for clarity):
...
- **CRP representation**: `dCRP(alpha, size = N)` uses the Chinese
  Restaurant Process to assign each person to a cluster. NIMBLE handles the
  partition prior directly; DP-measure weights and atoms are recovered later
  for density reconstruction through `getSamplesDPmeasure()`.
...
pattern to avoid executing `nimbleFunction()` at package load time.

```{r component-table, echo = FALSE}
comp_table <- data.frame(
  Component = c("dBernoulliVector", "logProb_summer", "sampler_centered"),
  Type = c("Distribution", "Sampler", "Sampler"),
  Purpose = c(
    "Vectorized Bernoulli likelihood for constrained_item identification",
    "Log-probability monitoring: tracks logProb(all), logProb(params), logLik(data)",
    "Joint adaptive MH for correlated (log_lambda, gamma) in SI parameterization"
  ),
  When_Used = c(
    "constrained_item with 2PL only; 3PL constrained_item is rejected",
    "By default when log-probability monitoring is enabled",
    "2PL/3PL SI with unconstrained or constrained_ability identification"
  ),
  stringsAsFactors = FALSE
)
knitr::kable(comp_table,
             caption = "Custom NIMBLE components registered by DPMirt.",
             align = "llll")
```

### dBernoulliVector

For `constrained_item` identification in 2PL models, the likelihood for each
person's entire response vector is evaluated jointly:
...
likelihood. DPMirt currently rejects `constrained_item` for 3PL models; use
`unconstrained` for 3PL by default.
...
the log-probability of a set of nodes at each MCMC iteration.
Three instances are configured:
...
The `myLogLik` trace is used for:
- Visual convergence checking (trace plots).
- Scalar log-likelihood diagnostics. It is not a pointwise WAIC fallback.
...
`delta` draws and item/trace plots rather than this scalar parameter trace.

### WAIC Provenance

When WAIC is enabled, DPMirt asks NIMBLE to compute online conditional WAIC
via `configureMCMC(enableWAIC = TRUE)` and extracts it with
...
mean-of-run scalar for compatibility. Before interpreting WAIC comparisons,
check `dpmirt_diagnostics(fit)$waic_aggregation`; values marked
`mean_of_chain_waic` are provenance summaries, not native pooled posterior
WAIC.

### sampler_centered

The centered sampler is intended to improve mixing in the
slope-intercept (SI) parameterization of 2PL and 3PL models.
It addresses the strong posterior correlation between $\log\lambda_i$
and $\gamma_i$ by making a joint proposal:
...
The centering mean $\bar\eta$ is updated each iteration to `mean(eta)`.
Adaptive scaling targets an acceptance rate of 0.44 (optimal for
univariate random-walk MH).

> **When is it used?** Only for 2PL/3PL models with SI parameterization
> and `identification = "unconstrained"` or `"constrained_ability"`.
> For the IRT parameterization, NIMBLE's default samplers are sufficient
> because the correlation structure is different.

The `dpmirt_compile()` function automatically enables the centered
sampler when `use_centered_sampler = "auto"` (the default).
In the current implementation, the centered sampler is added for SI
`log_lambda`/`gamma` pairs; inspect `compiled$mcmc_conf$printSamplers()` if
you need to verify whether default `gamma` samplers also remain in a custom
configuration.

## Compile-Once, Sample-Many Pattern

NIMBLE compilation translates the model and MCMC configuration into C++
code, compiles it, and loads the resulting shared library.
This is the most expensive step in the pipeline.

DPMirt separates compilation from sampling, allowing you to compile once
and then run additional sampling while the compiled object is live. Repeated
`dpmirt_sample(compiled, ...)` calls reset sample storage by default; use
`reset = FALSE` or `dpmirt_resume()` to extend the same compiled MCMC state.
For a pooled chain-labeled one-step fit, use `dpmirt(..., nchains = k)`.
...
timing <- data.frame(
  Operation = c("dpmirt_spec()", "dpmirt_compile()", "dpmirt_sample(10K)",
                "dpmirt_rescale()", "dpmirt_estimates()"),
  Typical_Time = c("< 1 sec", "30-120 sec", "10-60 sec",
                    "< 1 sec", "< 1 sec"),
  Bottleneck = c("Code generation", "C++ compilation",
                 "MCMC iterations", "Matrix operations",
                 "Quantile computation"),
  stringsAsFactors = FALSE
)
knitr::kable(timing,
             caption = "Typical timing for each pipeline stage (Rasch, N=200, I=25).",
             align = "lll")
```
...
preserve DPMirt's required monitors and log-probability samplers. List-based
sampler schemas are intentionally reserved.

> **Serialization caveat:** Compiled NIMBLE objects contain external
> C++ pointers that **cannot be serialized** across R sessions.
> If you call `saveRDS(compiled, ...)` and then `readRDS(...)` in a
> new session, the pointers will be dead and all MCMC operations will
> fail.  Instead, save the `dpmirt_spec` object and recompile:
>
> ```r
...
DPMirt's default is model-specific: Rasch defaults to `constrained_item`,
while 2PL and 3PL default to `unconstrained`. When the model is fitted with
`identification = "unconstrained"`, DPMirt applies rescaling to the posterior
samples afterwards.

### Rasch: Location Shift Only

The Rasch model has only location indeterminacy.
For each MCMC iteration $s$, the rescaling is:
...
The 2PL model has both location and scale indeterminacy.
...
Note the $+$ sign for theta rescaling (vs. $-$ in IRT parameterization).
...
mixing if the location/scale dimensions are poorly identified.
In practice, DPMirt's default priors provide sufficient regularization.

## DP Density Reconstruction

For DPM models, DPMirt can reconstruct the posterior predictive density
of the latent trait.
This follows Paganin et al.'s `getSamplesDPmeasure()` approach.

### Algorithm

For each post-burn-in MCMC iteration $s$:

1. **Extract** the stick-breaking weights $w_{s,1}, \ldots, w_{s,K_s}$
   and atoms $(\tilde\mu_{s,k}, \tilde\sigma^2_{s,k})$ from the CRP
   posterior via NIMBLE's `getSamplesDPmeasure()`.
...
(unrescaled) scale. To evaluate it on the rescaled theta scale, the current
implementation shifts the grid by the iteration-specific location shift:

$$
\hat{f}_{\text{rescaled}}(x) = f_s(x + \text{location}_s)
$$

The Jacobian is 1 for a location shift, so no density adjustment is needed.
For 2PL/3PL IRT and SI parameterizations, a full transformed-scale density
would also require scale/Jacobian handling:
...
2PL/3PL fits should be treated as diagnostic rather than definitive density
estimates.

### Using `dpmirt_dp_density()`

```{r dp-density-usage, eval = FALSE}
...
> **Note on computation cost:** `dpmirt_dp_density()` internally rebuilds
> a NIMBLE model/MCMC, loads retained DP draws through public `modelValues`
> accessors, and calls `getSamplesDPmeasure()`.  The `dpmirt()` function runs
> this automatically when `compute_dp_density = TRUE` (the default for DPM
> models). If you want to skip it, set `compute_dp_density = FALSE` and call
> `dpmirt_dp_density(fit)` later.
> Manual recomputation requires the live model specification retained with the
> compiled model reference; saved compact RDS fixtures can carry a precomputed
> compact `dp_density`, but cannot resurrect expired compiled C++ pointers.

## MCMC Pipeline Internals

The following diagram shows the complete data flow from raw data to
the final `dpmirt_fit` object:
...
DPMirt uses a k-means clustering strategy (following Paganin et al.)
for initializing DPM cluster assignments:
...
Since theta has $N$ dimensions (one per person), it can dominate memory
for large samples.
Setting `thin2 > thin` reduces storage:

```{r differential-thinning, eval = FALSE}
...
```{r custom-sampler-example, eval = FALSE}
...
| All persons in one cluster | alpha too small | Increase mu\_K or use "low" confidence |
...
|:---------|:------------|
| [Quick Start](quick-start.html) | End-to-end pipeline without worrying about internals |
| [Prior Elicitation](prior-elicitation.html) | Optional DPprior calibration of $\alpha$ and Gamma(1,3) fallback |
| [Simulation Study](simulation-study.html) | Full factorial comparison of methods and priors |
