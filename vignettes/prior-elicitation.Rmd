...
3. **DPprior integration** --- several routes to set $\alpha$ in DPMirt.
...
conservative default to fully manual DPprior workflows. DPprior is optional:
when it is unavailable, DPMirt falls back to $\text{Gamma}(1, 3)$.
...
This path calls `dpmirt_alpha_prior()` only when `prior = "dpm"`,
`alpha_prior = NULL`, and `mu_K` is not `NULL`. If `mu_K = NULL`, the
one-step wrapper uses the conservative $\text{Gamma}(1, 3)$ default.
...
cat("DPprior not installed --- return_fit = TRUE falls back to c(a = 1, b = 3).\n")
cat("Install DPprior to receive a full DPprior_fit object with diagnostics.\n")
```
...
cat("DPprior not installed; confidence levels are not calibrated.\n")
cat("DPMirt uses the conservative fallback alpha ~ Gamma(1, 3).\n")
```
...
> cluster structure.  The confidence levels target the same expected
> cluster count, but the fitted Gamma mean for $\alpha$ can shift slightly;
...
`mu_K = max(3, ceiling(log(N)))` used by `dpmirt_alpha_prior(N)`, but the
explicit `confidence = "low"` choice is deliberately more diffuse than the
function's default `confidence = "medium"`. The one-step `dpmirt()` wrapper
does not auto-elicit unless you set `mu_K`; with `mu_K = NULL` and
`alpha_prior = NULL`, it uses the conservative $\text{Gamma}(1, 3)$ default.
...
> itself. WAIC can help assess item-response prediction, but differences are
> often small and do not by themselves establish ability-distribution
> recovery.
...
[Posterior Summaries](posterior-summaries.html) for a detailed analysis of
how reliability moderates both estimator choice and prior choice.
...
count or want to inspect the full DPprior fit.
...
|:---------|:------------|
| [Quick Start](quick-start.html) | End-to-end pipeline with data simulation, fitting, and estimation |
| [Simulation Study](simulation-study.html) | Full factorial study comparing Normal vs. DPM priors under different conditions |
| [NIMBLE Internals](nimble-internals.html) | How DPMirt generates NIMBLE code, compiles, and samples |
